...
class Bot:
    ...
    def update(self, level):
        ...
        for col in range(15):
            for lig in range(15):
                x, y = Convert(lig, col)
                if (self.x > 560 or self.x < 0 or self.y > 560 or self.y < 0) or (
                    level[lig][col] == 5 and x == self.x and y == self.y
                ):
                    return "DEAD"
                ...
                if (
                    ...
                    return self.possibleDirection(lig, col, level)
                if (
                    (level[lig][col] == 4 or level[lig][col] == 2)
                    and x == self.x
                    and y == self.y
                ):
                    return self.possibleDirection(lig, col, level)
    ...
    def updateBlock(self, level):
        for col in range(15):
            for lig in range(15):
                x, y = Convert(lig, col)
                if self.defineDir(level, lig, col) == 6 and x == self.x and y == self.y:
                    if lig > 0 and (
                        level[lig - 1][col] == 1
                        or self.defineTunnel(level, lig - 1, col) == 11
                    ):
                        ...
                        self.direction = "UP"
                if self.defineDir(level, lig, col) == 7 and x == self.x and y == self.y:
                    if lig < 14 and (
                        level[lig + 1][col] == 1
                        or self.defineTunnel(level, lig + 1, col) == 11
                    ):
                        ...
                        self.direction = "DOWN"
                if self.defineDir(level, lig, col) == 8 and x == self.x and y == self.y:
                    if col > 0 and (
                        level[lig][col - 1] == 1
                        or self.defineTunnel(level, lig, col - 1) == 12
                    ):
                        ...
                        self.direction = "LEFT"
                if self.defineDir(level, lig, col) == 9 and x == self.x and y == self.y:
                    if col < 14 and (
                        level[lig][col + 1] == 1
                        or self.defineTunnel(level, lig, col + 1) == 12
                    ):
                ...
                if (
                    ...
                    and x == self.x
                    and y == self.y
                    and self.direction != "STAY"
                ):
                    self.statusDir = self.statusDir + 1 % 4

                if (
                    (level[lig][col] == 13 or level[lig][col] == 14)
                    and x == self.x
                    and y == self.y
                    and self.direction != "STAY"
                ):
    ...
    def possibleDirection(self, lig, col, level):
        ...
        res = []
        if col < 14 and (
            (level[lig][col + 1] == 1 or self.defineTunnel(level, lig, col + 1) == 12)
        ):
            revres.append("RIGHT")
        if col > 1 and (
            (level[lig][col - 1] == 1 or self.defineTunnel(level, lig, col - 1) == 12)
            ...
            revres.append("LEFT")
        if lig < 14 and (
            (level[lig + 1][col] == 1 or self.defineTunnel(level, lig + 1, col) == 11)
        ):
            revres.append("DOWN")
        if lig > 0 and (
            (level[lig - 1][col] == 1 or self.defineTunnel(level, lig - 1, col) == 11)
